...
| 双 ADC 相位差接口 | 保留 `Get_PhaseDifference` 等兼容接口，当前未启用 |
...
**采样率动态计算**：`Phase_Get_TIM3_TriggerFreq_Hz()` 会自动检测 ETR 外部时钟模式，用 Si5351 频率 (1.024MHz) / TIM3 周期计算实际采样率。修改 TIM3 Period 后采样率自动同步，无需手动改代码。
...
### 4.3 void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
DMA 完成回调（中断上下文）。只做快照 + 重启 DMA，不在此做 FFT 计算。
...
修改 `Phase.c` 中 `FFT_App_Init()` 的 TIM3 Period：

```c
htim3.Init.Period = X - 1;  // 采样率 = 1.024MHz / X
```
...
4. **双 ADC 接口**：`Get_PhaseDifference` 等为旧兼容接口，当前未接入实际信号链路
