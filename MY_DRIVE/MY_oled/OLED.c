 ...
 ***********************************************************/
/*
 * 初始化命令数组（针对常见 SSD1306 128x64 OLED 模块）
 * 每个字节后面注释说明该命令/参数的含义。
 */
uint8_t CMD_Data[] = {
	0xAE,       // DISPLAY OFF: 关闭显示，初始化开始常用
	0xD5, 0x80, // SET DISPLAY CLOCK DIV: 设置显示时钟分频和振荡器(0x80 为常用推荐值)
	0xA8, 0x3F, // SET MULTIPLEX RATIO: 多路复用比 0x3F -> 63 (实际 MUX = 64 行，适用于 64 高度)
	0xD3, 0x00, // SET DISPLAY OFFSET: 显示偏移 0x00 (无偏移)
	0x40,       // SET START LINE: 起始行地址 0
	0xA1,       // SEGMENT REMAP: 段重映射（列映射），0xA1 通常实现左右翻转
	0xC8,       // COM OUTPUT SCAN DIRECTION: COM 扫描方向（上下翻转），0xC8 为常用值
	0xDA, 0x12, // SET COM PINS: COM 引脚配置，0x12 为 128x64 模块常用值
	0x81, 0xCF, // SET CONTRAST: 对比度/亮度，0xCF 为较高亮度设置
	0xD9, 0xF1, // SET PRE-CHARGE PERIOD: 预充电周期，0xF1 为常见组合
	0xDB, 0x40, // SET VCOMH: VCOMH 电压反选等级，0x40 为推荐值
	0xA4,       // DISPLAY ALL ON RESUME: 恢复显示 RAM 内容（不是全部点亮）
	0xA6,       // NORMAL DISPLAY: 正常显示（非反显），0xA7 为反显
	0x8D, 0x14, // CHARGE PUMP SET: 充电泵设置，0x14 通常表示启用内部升压
	0xAF        // DISPLAY ON: 打开显示，初始化序列末尾
};
