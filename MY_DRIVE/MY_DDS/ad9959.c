...
{
    length = length * 300; // 进一步增加延时，确保 H7 480M 环境下的稳定性
    while (length--) { __nop(); }
}
...
{
    uint8_t FR1_DATA[3] = {0xD3, 0x00, 0x00}; // 20倍频，开启VCO增益
    ...
    // 3. 产生 IO UPDATE 脉冲让 PLL 设置生效
    AD9959_IO_Update(); 
    delay_9959(2000); // 关键：给 PLL 锁定留出极充足的时间
...
void Write_Frequence(uint8_t Channel, uint32_t Freq)
{
    ...
    // 基准频率补正：2^32 / 500,000,000 ≈ 8.589934592
    // 如果 PLL 锁定成功，主频就是 500MHz，系数应为 8.5899...
